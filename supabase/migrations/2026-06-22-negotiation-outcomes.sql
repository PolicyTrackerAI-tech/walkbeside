...
--
-- Payment flow is NOT touched. amount_paid_cents below is the price the family
-- paid the FUNERAL HOME — an analytics figure unrelated to the Stripe $199
-- advocate fee (which lives in negotiations.fee_cents).
...
comment on column public.negotiation_outreach.chosen is
  'True for the single home the family selected. Set via /admin/outcomes — the selection moment itself lives in the off-limits Stripe payment flow.';
...
comment on column public.negotiations.amount_paid_cents is
  'Analytics only: cents the family paid the funeral home. Unrelated to the Stripe $199 advocate fee (negotiations.fee_cents).';
