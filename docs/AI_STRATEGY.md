...
> `docs/CLAIMS_VALIDATION.md` first. Outreach today is a deterministic static
> template (safe by construction); if any AI email generation is added, gate it
> behind a pre-send guard (ready implementation on the `prelaunch-consolidation`
> branch: `lib/negotiation/guard.ts`).
